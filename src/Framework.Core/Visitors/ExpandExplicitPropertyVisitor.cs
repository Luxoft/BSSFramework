...
{
    private static readonly IDictionaryCache<Tuple<Type, MemberInfo>, PropertyInfo> ExpandPropertyCache =
            new DictionaryCache<Tuple<Type, MemberInfo>, PropertyInfo>(
                                                                       memberPair =>
                                                                       {
                                                                           var pathsRequest =
                                                                                   from property in
                                                                                           (memberPair.Item2 as PropertyInfo)
                                                                                           .ToMaybe()
                                                                                   from implementedProperty in memberPair.Item1
                                                                                           .GetImplementedProperty(property)
                                                                                           .ToMaybe()
