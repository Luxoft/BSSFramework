...
{
    private readonly HashSet<IgnoreLink> ignoreDomainTypeLinksHash;
    ...
    private void ProcessViews(RequiredRefContext refContext)
    {
        foreach (var databaseGrouped in refContext.RequeredColumns.GroupBy(z => ((Table)z.Parent).Parent))
        {
            ...
            foreach (var view in database.Views.Where(z => !z.IsSystemObject))
            {
    ...
    private void ApplyRequered(RequiredRefContext refContext, Table table, IEnumerable<string> columnNames)
    {
        ...
        var columns = table.Columns.Where(z => columnNamesHashSet.Contains(z.Name.ToLower())).ToList();
        ...
        var indexed = table.Indexes
                           .Where(z => z.IndexedColumns.Any(q => columnNamesHashSet.Contains(q.Name.ToLower()))).ToList();
    ...
    private class RequiredRefContext(IDatabaseScriptGeneratorContext contex)
    {
