...
public static class CommonPeriodExtensions
{
    ...
    /// <param name="date">Дата, которую нужно проверить на вхождение в период</param>
    /// <returns>true, если дата содержится в периоде, в противном случае — false</returns>
    public static bool NativeContains(this Period period, DateTime date) => period.NativeStartDate <= date && (period.NativeEndDate == null || date <= period.NativeEndDateValue);
    ...
    /// <param name="date">Дата, которую нужно проверить на вхождение в период</param>
    /// <returns>true, если дата не является null и дата содержится в периоде, в противном случае — false</returns>
    public static bool NativeContains(this Period period, DateTime? date) => date.MaybeNullable(z => period.NativeContains(z));
    ...
    public static bool ContainsWithoutEndDate(this Period period, DateTime date) => period.StartDate <= date && (period.EndDate == null || date < period.EndDateValue);
    ...
    public static string GetInMonthAndYearStrRus(this Period period) => period.StartDate.GetInMonthAndYearStrRus();
    ...
    public static int GetWeekNumber(this Period period) => period.GetWeekNumber(Thread.CurrentThread.CurrentCulture.Name);
    ...
    public static string GetMonthAndYearStrRoman(this Period period) => MonthFormatter.GetMonthAndYearStrRoman(period);
