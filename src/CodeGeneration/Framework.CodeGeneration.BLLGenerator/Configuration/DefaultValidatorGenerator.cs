    ...
    where TConfiguration : class, IBLLGeneratorConfiguration<IBLLGenerationEnvironment>
{
    ...
    protected virtual IEnumerable<ValidatorPairExpr> GetClassValidators()
    {
        ...
        foreach (var attr in this.Configuration.Environment.MetadataProxyProvider.Wrap(this.DomainType).GetCustomAttributes<ClassValidatorAttribute>())
        {
    ...
    protected virtual IEnumerable<ValidatorPairExpr> GetPropertyValidators(PropertyInfo property)
    {
        ...
        var propMetadata = this.Configuration.Environment.MetadataProxyProvider.Wrap(property);
            ...
            if (args != null)
            {
                ...
                if (validatorSourceType.IsAssignableFrom(this.DomainType) && validatorPropertyType.IsAssignableFrom(property.PropertyType))
                {
                    var methodRef = typeof(PropertyValidatorExtensions).ToTypeReferenceExpression().ToMethodReferenceExpression(
                        "Unbox",
                        [this.DomainType, validatorSourceType, property.PropertyType, validatorPropertyType]);
            ...
            if (args != null)
            {
                ...
                if (validatorSourceType.IsAssignableFrom(this.DomainType))
                {
                    var methodRef = typeof(ClassValidatorExtensions).ToTypeReferenceExpression()
                                                                    .ToMethodReferenceExpression("Unbox", [this.DomainType, validatorSourceType]);
    ...
    {
        var request = from attr in this.Configuration.Environment.MetadataProxyProvider.Wrap(this.DomainType).GetCustomAttributes<ClassValidatorAttribute>()
