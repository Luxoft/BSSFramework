...
public static class Extensions
{
    ...
    private class CodeFileGeneratorDecorator(IFileGenerator<ICodeFile, CodeDomRenderer> source, string suffix) : IFileGenerator<ICodeFile, CodeDomRenderer>
    {
        public CodeDomRenderer Renderer => source.Renderer;
        ...
        public IEnumerable<ICodeFile> GetFileGenerators()
        {
            ...
            foreach (var valueTuple in pairs)
            {
                ...
                yield return new CodeFile(valueTuple.mainGenerator.generator.Filename + suffix + ".Generated", resultNameSpace);
            }
        }
    }

    private class CodeFile(string fileName, CodeNamespace renderedData) : ICodeFile
    {
        public string Filename { get; } = fileName;

        public CodeNamespace GetRenderData() => renderedData;
    }
