    ...
    where TConfiguration : class, IServerDTOGeneratorConfiguration<IServerDTOGenerationEnvironment>
{
    ...
    private DomainObjectAccessAttribute GetDomainObjectAttribute(IServerPropertyAssigner propertyAssigner, PropertyInfo property, bool isEdit)
    {
        ...
        if (isEdit)
        {
            return this.Configuration.Environment.MetadataProxyProvider.GetProxy(property).GetEditDomainObjectAttribute().FromMaybe(() => $"Edit operation for property \"{property.Name}\" in domainObject \"{propertyAssigner.DomainType.Name}\" not found");
        }
        else
        {
            return (this.Configuration.Environment.MetadataProxyProvider.GetProxy(property).GetViewDomainObjectAttribute()
                 ?? this.Configuration.Environment.MetadataProxyProvider.GetProxy(propertyAssigner.DomainType).GetViewDomainObjectAttribute())
