...
{
    [HttpPost]
    public PrincipalFullDTO GetCurrentPrincipal() => this.Evaluate(
        DBSessionMode.Read,
        evaluateData => evaluateData.Context.CurrentPrincipalSource.CurrentUser.ToFullDTO(evaluateData.MappingService));
    ...
    public IEnumerable<PrincipalVisualDTO> GetVisualPrincipalsWithoutSecurity() =>
        this.Evaluate(
            DBSessionMode.Read,
            evaluateData =>
                evaluateData.Context.SecurityService.HasAccess(DomainSecurityRule.AnyRole)
