...
{
    [TestFixture]
    public sealed class GenerationLambdaProcessorToTests : TestFixtureBase
    {
        ...
        public void PublicSurface_NullArguments_ArgumentNullException()
        {
            ...
            // Assert
            assertion.Verify(typeof(GenerationLambdaProcessorTo<ITestBLLContext>));
        }
        ...
        public void InvokeFuncValue_InvalidResultType_Exception()
        {
            ...
            call.Should().Throw<SubscriptionServicesException>()
                .WithMessage("Generation lambda for subscription \"S-000\" has failed with message \"Unable cast Generation lambda FuncValue result 'zzz' to required type System.Collections.Generic.IEnumerable`1[Framework.Notification.NotificationMessageGenerationInfo]\".");
        }
            ...
            Func<object, object, object> funcValue = null) where T : class
        {
            ...
            var subscription = this.Fixture
                .Build<Subscription>()
                .With(s => s.Code, "S-000")
                .With(s => s.Generation, lambdaString != null || funcValue != null ? lambda : null)
                .Create();

            // Act
            var processor = new GenerationLambdaProcessorTo<string>(context, this.parserFactory);
