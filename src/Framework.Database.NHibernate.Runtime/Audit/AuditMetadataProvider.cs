...
{
    private IList<Type> filledTypes;

    private readonly string auditTableSuffix;

    private readonly IEntityTrackingRevisionListener entityTrackingRevisionListener;
        ...
        IEntityTrackingRevisionListener entityTrackingRevisionListener)
    {
        ...
        this.auditTableSuffix = auditTableSuffix;
        this.entityTrackingRevisionListener = entityTrackingRevisionListener;
    ...
    public IDictionary<Type, IEntityMeta> CreateMetaData(Configuration nhConfiguration)
    {
        ...
        entityMeta.AddClassMeta(new RevisionEntityAttribute { Listener = this.entityTrackingRevisionListener });
    ...
    private HbmMapping CreateRevisionInfoMappingDocument()
    {
        ...
        mapper.Class<TAuditRevisionEntity>(mapping =>
        {
            ...
            mapping.Table(nameof(AuditRevisionEntity));
    ...
    {
        if (!this.filledTypes.Contains(type) && !type.IsAbstract)
        {
            if (!dicToFill.ContainsKey(type))
            {
                dicToFill[type] = new EntityMeta();
