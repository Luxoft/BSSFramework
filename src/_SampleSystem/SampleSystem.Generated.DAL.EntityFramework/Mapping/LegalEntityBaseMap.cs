...
public class LegalEntityBaseMap : SampleSystemBaseMap<LegalEntityBase>
{
    public override void Configure(EntityTypeBuilder<LegalEntityBase> builder)
    {
        ...
        builder.OwnsOne(x => x.AribaStatus, status => { status.Property(x => x.Date).HasColumnName("aribaStatusdate"); status.Property(x => x.Description).HasColumnName("aribaStatusdescription").HasMaxLength(int.MaxValue); status.Property(x => x.Type).HasColumnName("aribaStatustype"); });
    }
}
