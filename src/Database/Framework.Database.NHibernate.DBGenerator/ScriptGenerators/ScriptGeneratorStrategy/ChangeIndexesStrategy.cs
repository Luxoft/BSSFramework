...
/// </summary>
internal class ChangeIndexesStrategy(DatabaseScriptGeneratorStrategyInfo parameter) : ScriptGeneratorStrategyBase(parameter)
{
    /// <summary>
    ...
    private void ChangeIndexesForRemovableColumns(DomainTypeMetadata domainTypeMetadata)
    {
        ...
        var previousIndexies = table.Indexes
                                    .Where(z => z.IndexedColumns.Any(q => q.Name.EndsWith(this.Parameter.PreviousPostfix)))
                                    .ToList();
        ...
        {
            var indexedColumn = table.Indexes.SelectMany(
                                         z => z.IndexedColumns.Where(q => q.Name == removableColumn.Name))
                                     .ToList();
    ...
    private bool GenerateForeignKeys(DomainTypeMetadata domainTypeMetadata, Table table)
    {
        ...
        foreach (var reference in domainTypeMetadata.GetPersistentReferenceFields().SelectMany(MapperFactory.GetMapping))
        {
            ...
            var existingForeignKey = table.ForeignKeys.FirstOrDefault(z => z.Name.Equals(foreignKeyName, StringComparison.CurrentCultureIgnoreCase));
