...
public partial class TargetSystemBLL
{
    ...
    private TargetSystem Register(string targetSystemName, bool isBase, bool isMain, bool isRevision, Guid id, IReadOnlyDictionary<Guid, Type> domainTypes)
    {
        ...
        foreach (var newItem in mergeResult.AddingItems)
        {
            ...
            if (!isBase)
            {
                newItem.Value.GetEventOperations(typeof(EventOperation)).Foreach(value => new DomainTypeEventOperation(newDomainType, value));
            }
        ...
        foreach (var mergePair in mergeResult.CombineItems)
        {
            ...
            var mergeEventResult = domainType.EventOperations.GetMergeResult(type.GetEventOperations(typeof(EventOperation)), operation => operation.Name, value => value.ToString());
