...
public class SqlServer(Server server, IOptions<AutomationFrameworkSettings> automationFrameworkSettingsOptions) : ISqlServer
{
    public void Refresh() => server.Refresh();

    public ISqlServerDatabase? TryGetDatabase(string initialCatalog) => server.Databases[initialCatalog] is { } db ? new SqlServerDatabase(db) : null;
