...
public class AuditDatabaseScriptGenerator : IDatabaseScriptGenerator
{
    ...
    private static IEnumerable<string> CreateCreateOrAlterTableScripts(AuditTableGenerateContext context, bool createMode)
    {
        if (createMode)
        {
            return new AuditSqlScriptGenerator().GenerateCreateScripts(context);
        }

        return new AuditSqlScriptGenerator().GenerateUpdateScripts(context);
    }


    private IEnumerable<IPersistentIdentifierGenerator> IterateGenerators(Configuration configuration, global::NHibernate.Dialect.Dialect dialect)
    {
