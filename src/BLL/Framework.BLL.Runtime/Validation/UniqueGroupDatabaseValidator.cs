...
public class UniqueGroupDatabaseValidator(string? groupKey) : IDynamicClassValidator
{
    ...
    public IClassValidator GetValidator(Type domainObjectType, IServiceProvider serviceProvider)
    {
        ...
        var internalValidatorType = typeof(UniqueGroupDatabaseValidator<,,,>)
            .MakeGenericType(
                contextTypeData.ContextType,
                contextTypeData.PersistentDomainObjectBaseType,
                domainObjectType,
        ...
        IEnumerable<PropertyInfo> properties)
    {
        ...
        var newValuesArrExpr = Expression.NewArrayInit(
            ...
            properties
                .Select(property =>
                            Expression.Property(domainObjectParameter, property)
                                      .Pipe(propExpr =>
                                                propExpr.Type.IsClass
                                                    ? (Expression)propExpr
    ...
    private LambdaExpression GetGetFilterExpression(Type domainObjectType, Type identType, IEnumerable<PropertyInfo> properties)
    {
        ...
        var duplicateFilter = properties.Aggregate(
            getBinaryExpr(idProp, Expression.NotEqual),
            (filter, property) =>
