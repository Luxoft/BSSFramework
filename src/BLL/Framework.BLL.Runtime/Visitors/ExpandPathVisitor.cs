...
{
    private readonly IDictionaryCache<Tuple<Type, MemberInfo>, PropertyPath?> expandPathCache = new DictionaryCache<Tuple<Type, MemberInfo>, PropertyPath?>(memberPair =>
    {
        var pathsRequest = from property in (memberPair.Item2 as PropertyInfo).ToMaybe()
                           ...
                           from expandPath in propertyPathService.TryGetExpandPath(property).ToMaybe()
    ...
    protected override Expression VisitMember(MemberExpression node)
    {
        var request = from expr in node.Expression.ToMaybe()
                      ...
                      from properties in this.expandPathCache.GetValue(expr.Type, node.Member).ToMaybe()
    ...
    private static readonly HashSet<ExpressionType> LiftableOperators =
    [
        ExpressionType.Or,
