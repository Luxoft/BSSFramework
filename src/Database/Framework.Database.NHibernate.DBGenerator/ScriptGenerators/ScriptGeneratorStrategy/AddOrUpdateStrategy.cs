...
internal class AddOrUpdateStrategy(DatabaseScriptGeneratorStrategyInfo parameter) : ScriptGeneratorStrategyBase(parameter)
{
    /// <summary>
        ...
        foreach (var sqlMapping in sqlMappings.OrderBy(z => z.Name))
        {
            ...
            if (!this.Parameter.DataTypeComparer.Equals(column.DataType, sqlMapping.SqlType))
            {
                ...
                if (column.Nullable != sqlMapping.IsNullable && !this.Parameter.DataTypeComparer.Equals(column.DataType, sqlMapping.SqlType))
                {
                    ...
                    table.ForeignKeys
                         .Where(z => z.Columns.Any(q => q.Name == column.Name))
                         .ToList()
                         .Foreach(z => z.Drop());

                    table.Indexes
                         .Where(z => z.IndexedColumns.Any(q => q.Name == column.Name))
                         .ToList()
                         .Foreach(z =>
                         {
                             z.Drop();
