...
//            return domainObject => baseQuery.Any(permission => permission.DenormalizedItems.Any(denormalizedPermissionItem => filterExpression.Eval(domainObject, permission, denormalizedPermissionItem)));
//        }


//        public abstract Expression<Func<TDomainObject, IPermission<TIdent>, IDenormalizedPermissionItem<TIdent>, bool>> GetSecurityFilterExpression(HierarchicalExpandType expandType);
...
//                var grandAccessIdent = this.Factory.AuthorizationSystem.GrandAccessIdent;

//                var fullAccessFilter = ExpressionHelper.Create((IPermission<TIdent> permission) => permission.DenormalizedItems.Where(i => i.EntityType.Name == securityObjectName).Select(i => i.EntityId).Contains(grandAccessIdent));
...
//                                permission.DenormalizedItems.Any(i => i.EntityType.Name == securityObjectName && securityIdents.Contains(i.EntityId)));
//                }
//                else
//                {
...
//        public class SecurityByIdentsExpressionBuilder<TSecurityContext> : SecurityByIdentsExpressionBuilderBase<TSecurityContext, SecurityPath<TDomainObject>.SecurityPathByIdents<TSecurityContext>>
//            where TSecurityContext : class, ISecurityContext
//        {
//            public SecurityByIdentsExpressionBuilder(SecurityExpressionBuilderFactory<TPersistentDomainObjectBase, TIdent> factory, SecurityPath<TDomainObject>.SecurityPathByIdents<TSecurityContext> path)
//                : base(factory, path)
//            {
//            }
...
//            public override Expression<Func<TDomainObject, IPermission<TIdent>, IDenormalizedPermissionItem<TIdent>, bool>> GetSecurityFilterExpression(HierarchicalExpandType expandType)
//            {
//                throw new NotImplementedException();
//            }
//        }
...
//                return (domainObject, _, _) => securityFilter.Eval(domainObject);
//            }
...
//                                           ? ExpressionHelper.Create((TIdent itemId, TIdent permissionEntityId) => eqIdentsExpr.Eval(permissionEntityId, itemId))
//                                           : ExpressionHelper.Create((TIdent itemId, TIdent permissionEntityId) => expandExpression.Eval(permissionEntityId).Contains(itemId)))
//                                   .ExpandConst()
//                                   .InlineEval();
...
//                        return (domainObject, _, denormalizedPermissionItem) =>

//                                       this.Path.SecurityPath.Eval(domainObject) == null

//                                           || containsItem.Eval(this.Path.SecurityPath.Eval(domainObject).Id, denormalizedPermissionItem.EntityId);
...
//                        return (domainObject, permission, denormalizedPermissionItem) => !this.Path.NestedObjectsPath.Eval(domainObject).Any()

//                                                          || this.Path.NestedObjectsPath.Eval(domainObject).Any(nestedObject => baseFilter.Eval(nestedObject, permission, denormalizedPermissionItem));

//                    case ManySecurityPathMode.AnyStrictly:

//                        return (domainObject, permission, denormalizedPermissionItem) => this.Path.NestedObjectsPath.Eval(domainObject).Any(nestedObject => baseFilter.Eval(nestedObject, permission, denormalizedPermissionItem));

//                    case ManySecurityPathMode.All:

//                        return (domainObject, permission, denormalizedPermissionItem) => this.Path.NestedObjectsPath.Eval(domainObject).All(nestedObject => baseFilter.Eval(nestedObject, permission, denormalizedPermissionItem));
...
//                               leftFilter.Eval(domainObject, permission, denormalizedPermissionItem) && rightFilter.Eval(domainObject, permission, denormalizedPermissionItem);
//            }
