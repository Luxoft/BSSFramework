...
public class AuditReaderPatchedFactory(ISession session) : IAuditReaderPatchedFactory
{
    public IAuditReaderPatched Create()
    {
        ...
        var auditEventListener = listeners.PostInsertEventListeners.OfType<AuditEventListener>().SingleOrDefault()
                                 ?? listeners.PostUpdateEventListeners.OfType<AuditEventListener>().SingleOrDefault();
